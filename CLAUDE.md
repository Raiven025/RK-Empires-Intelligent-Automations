...
- Both standalone login pages link to `https://app.rk-empires.com/forgot-password`, passing `portal=client` or `portal=admin` for presentation only. Recovery uses the existing platform API and users database; authorization never comes from the portal query parameter.
